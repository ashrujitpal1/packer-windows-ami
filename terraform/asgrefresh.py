...
def get_launch_configuration_id(asg_name):
    try:
        asg = asg_client.describe_auto_scaling_groups(
            AutoScalingGroupNames=[asg_name])
        # Make sure the Auto Scaling group exists
        ...
        if len(asg['AutoScalingGroups']) == 0:
            raise ValueError("Autoscaling group  : {} not found.".format(asg_name))
        asg_details = asg['AutoScalingGroups'][0]
        # Check if configuration exist into ASG
        if 'LaunchConfigurationName' in asg_details.keys():
            template_id = asg_details['LaunchConfigurationName']
            print(asg)
            return template_id
        elif 'MixedInstancesPolicy' in asg_details.keys():
            template_id = asg_details['MixedInstancesPolicy']['LaunchConfigurationName']['LaunchConfigurationSpecification']['LaunchConfigurationId']
            return template_id
        else:
            return None
...
def create_new_launch_configuration(lt_id, ami_id):
    try:
        launch_configurations = asg_client.describe_launch_configurations(
            LaunchConfigurationNames=[lt_id])
        response = asg_client.create_launch_configuration(
             LaunchConfigurationName=lt_id,
             ImageId=ami_id,
             InstanceType='t2.micro')
        logging.info("New Version of Launch Configuration created successfully.")
        print(response)
        return response['ResourceId']
    except ClientError as exp:
        logging.error("Error occured during new launch configuration creation.")
        raise exp        
